...
namespace Twitch
{
	...
	{
		// Token: 0x06000097 RID: 151 RVA: 0x0000FE64 File Offset: 0x0000E064
		public static void PopulateAssets()
		{
			bool flag = Assets.MainAssetBundle == null;
			if (flag)
			{
				using (Stream manifestResourceStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("Twitch.twitch"))
				{
					Assets.MainAssetBundle = AssetBundle.LoadFromStream(manifestResourceStream);
				}
			}
			using (Stream manifestResourceStream2 = Assembly.GetExecutingAssembly().GetManifestResourceStream("Twitch.Twitch.bnk"))
			{
